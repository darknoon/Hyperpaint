...
public struct StableDiffusionPipeline {
  ...
  /// Model to embeddings for an input image
  var encoder: Encoder?
  ...
  public func generateImages(
    prompt: String,
    inputLatent: MLShapedArray<Float32>? = nil,
    imageCount: Int = 1,
    stepCount: Int = 50,
    seed: Int = 0,
    ...
    var latents =
      inputLatent != nil
      ? Array(repeating: inputLatent!, count: imageCount)
      : generateLatentSamples(imageCount, stdev: stdev, seed: seed)
  ...
  /// - Parameters:
  ///   - prompt: Text prompt to guide sampling
  ///   - imageInput: Image prompt to guide sampling
  ///   - stepCount: Number of inference steps to perform
  ///   - imageCount: Number of samples/images to generate for the input prompt
  ///   - seed: Random seed which
  ///   - disableSafety: Safety checks are only performed if `self.canSafetyCheck && !disableSafety`
  ...
  public func generateImages(
    ...
    guard let encoder = encoder else {
      throw RunError.resources("Missing encoder")
    }
    guard let image = image else {
      return try generateImages(prompt: prompt, inputLatent: nil, imageCount: imageCount, stepCount: stepCount, seed: seed, disableSafety: disableSafety, progressHandler: progressHandler)
    }
    let inputDistribution = try encoder.encode(image)
    var inputLatent = MLShapedArray<Float32>(converting: inputDistribution.sample())
    // Very unoptimized!
    inputLatent = MLShapedArray<Float32>(scalars: inputLatent.scalars.map{$0 * 0.18215}, shape: inputLatent.shape)
    return try generateImages(prompt: prompt, inputLatent: inputLatent, imageCount: imageCount, stepCount: stepCount, seed: seed, disableSafety: disableSafety, progressHandler: progressHandler)
  }
